    ...
    void Update() {
        if (!paused) {
			if (pausedTime != 0) {
                ...
                started = GameClock.Time - delta;
                pausedTime = 0;
            }
            label.text = TimingToString(GetDuration(), false);
            if (!label.enabled) { label.enabled = true; }
        } else {
            if(pausedTime == 0) { pausedTime = GameClock.Time; label.text = TimingToString(GetDuration(), true); }
            label.enabled = (System.Environment.TickCount & (1<<blinkDuration)) == 0;
        }
    }
    int blinkDuration = 9;

    public int GetDuration() {
        return (int)(GameClock.Time - started);
    }
}
