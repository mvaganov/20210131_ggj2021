...
namespace NonStandard.Cli {
	public static class ManageUI
	{
		...
		{
			public ColorRGBA Background = new Color(0, 0, 0, 0.5f);
			public ColorRGBA Text = new Color(1, 1, 1);
			public ColorRGBA ErrorText = new Color(1, .5f, .5f);
			public ColorRGBA SpecialText = new Color(1, .75f, 0);
			public ColorRGBA ExceptionText = new Color(1, .5f, 1);
			public ColorRGBA Scrollbar = new Color(1, 1, 1, 0.5f);
			public ColorRGBA UserInput = new Color(.75f, .875f, .75f);
			public ColorRGBA UserSelection = new Color(.5f, .5f, 1, .75f);
			private string _cachedUInputHex;
			private ColorRGBA _cachedUInputColor;
			public string UserInputHex {
				get {
					if (_cachedUInputColor != UserInput)
					{
						_cachedUInputHex = UserInput.ToHexString();
						_cachedUInputColor = UserInput;
					}
					return _cachedUInputHex;
				}
			}
			public string ErrorTextHex { get { return ErrorText.ToHexString(); } }
			public string SpecialTextHex { get { return SpecialText.ToHexString(); } }
			public string ExceptionTextHex { get { return ExceptionText.ToHexString(); } }
		}
